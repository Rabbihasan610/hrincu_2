                        ...
                        <table class="table">
                            ...
                            <tbody>
                                @forelse($services as $service)
                                    <tr>
                                        <td> {{ $loop->index + 1 }} </td>
                                        <td> <img src="{{ getImage(getFilePath('sector') . '/' . $service->image) }}" alt="image" class="rounded" style="width: 50px"> </td>
                                        <td> {{ $service->lang('title') }} </td>
                                        <td> {{ Str::limit($service->lang('description'), 50) }} </td>
                                        <td>
                                            <x-status :data="$service" :url="route('admin.sectors.status', $service->id)" />
                                        </td>
                                        <td>
                                            <div class="btn-group">
                                                ...
                                                <ul class="dropdown-menu dropdown-menu-end">
                                                    ...
                                                    <li>
                                                        <a href="{{ route('admin.sectors.edit', $service->id) }}">
                                                            <i class="bi bi-pencil"></i>@lang('Edit')
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a href="{{ route('admin.sectors.duplicate', $service->id) }}">
                                                            <i class="bi bi-clipboard-plus"></i>@lang('Duplicate')
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <a href="javascript:void(0)" class="confirmationBtn"
                                                            data-action="{{ route('admin.sectors.delete', $service->id) }}"
                                                            data-question="@lang('Are you sure to delete this Sectors?')">
...
@push('breadcrumb-plugins')
    <div class="flex-wrap gap-3 d-flex">
        ...
        <a href="{{ route('admin.sectors.create') }}" class="btn btn-primary"> <i class="fa-solid fa-plus"></i>@lang('Add New')</a>
    </div>
@endpush
